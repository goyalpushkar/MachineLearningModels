        ...
        for c in range(C):
            for w in range(W // self.s):
                for h in range(H // self.s):
                    out[c, w, h] = np.max(
                        self.inputs[c, w * self.s:w * self.s + self.pool, h * self.s:h * self.s + self.pool])
        return out
...
class Net:
    def __init__(self):
        ...
        self.layers = []
        self.layers.append(
            Convolution2D(inputs_channel=1, num_filters=6, kernel_size=5, padding=0, stride=2, learning_rate=lr,
                          name='conv1'))
...
"""
print('Loading data......')
sub_train_images, sub_train_labels, sub_test_images, sub_test_labels = init_subset('0000') # input your ASUID last 4 digits here to generate the subset samples of MNIST for training and testing
...
def evaluate(net, images, labels):
    ...
    for batch_index in range(0, images.shape[0], batch_size):
        ...
        x = data[0]  # images[batch_index]
        y = label[0]  # labels[batch_index]

        # print("x: \n", len(x), x.shape,  x.shape[0], x, "y: \n", len(y), y.shape, y.shape[0], y)
        # forward pass
        ...
        # print("Loss and accuracy calcualted")
        total_labels += 1

        # print("Loss", loss)  # .item()  loss.item()
        # print("Accuracy", acc, total_labels)

    loss = loss / len(images)
    acc = acc / total_labels  # 100. *
...
for e in range(epoch):
    ...
    for batch_index in range(0, sub_train_images.shape[0], batch_size):
        ...
        print('=== Iter:{0:d} === Remain: {1:d} Hrs {2:d} Mins {3:d} Secs ==='.format(int(batch_index + batch_size),
                                                                                      int(hrs), int(mins), int(secs)))

    # Print out the Performance
    train_acc, train_loss = evaluate(net, sub_train_images,
                                     sub_train_labels)  # Use the evaluation code to obtain the training accuracy and loss
    test_acc, test_loss = evaluate(net, sub_test_images,
                                   sub_test_labels)  # Use the evaluation code to obtain the testing accuracy and loss
    ...
    print('=== Epoch:{0:d} Train Size:{1:d}, Train Acc:{2:.3f}, Train Loss:{3:.3f} ==='.format(e, train_size, train_acc,
                                                                                               train_loss))

    print('=== Epoch:{0:d} Test Size:{1:d}, Test Acc:{2:.3f}, Test Loss:{3:.3f} ==='.format(e, test_size, test_acc,
                                                                                            test_loss))

import matplotlib.pyplot as plt
def plot (x_data, y_data, y_label):
    plt.plot(x_data, '-o')
    plt.plot(y_data, '-o')
    plt.xlabel('epoch')
    plt.ylabel(y_label)
    plt.legend(['Train', 'Test'])
    plt.title('Train vs Test '+y_label)
...
final_train_accuracy = sum(train_accuracies)/ len(train_accuracies)
final_test_accuracy = sum(test_accuracies)/ len(test_accuracies)

print("Losses:", final_train_loss, final_test_loss)
print("Accuracies:", final_train_accuracy, final_test_accuracy)